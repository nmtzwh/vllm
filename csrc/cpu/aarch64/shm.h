...
__attribute__((target("+bf16"))) inline float32x4x2_t cvt_bf16_to_fp32(
    const bfloat16x8_t src) {
  float32x4x2_t y;
...
__attribute__((target("+bf16"))) inline bfloat16x8_t cvt_fp32_to_bf16(
    const float32x4x2_t src) {
  return vcvtq_high_bf16_f32(vcvtq_low_bf16_f32(src.val[0]), src.val[1]);
}

__attribute__((target("+bf16"))) inline void reduce_bf16_buffers(
    int start_elements,
    int num_elements,
    char* to_buffer,
    char** buffers,
...
#pragma omp parallel for
  for (int i = start_elements * element_size;
    ...
    float32x4x2_t inout_val =
        cvt_bf16_to_fp32(vld1q_bf16((const bfloat16_t*)(buffers[0] + i)));
    for (int j = 1; j < world_size; ++j) {
      const float32x4x2_t in_val =
          cvt_bf16_to_fp32(vld1q_bf16((const bfloat16_t*)(buffers[j] + i)));
...
inline void reduce_fp16_buffers(
    int start_elements,
    int num_elements,
    char* to_buffer,
    char** buffers,
...
inline void reduce_fp32_buffers(
    int start_elements,
    int num_elements,
    char* to_buffer,
    char** buffers,
...
inline void parallel_memcpy(void* to, void* from, size_t n_bytes) {
  const size_t aligned_bytes =
      n_bytes - (n_bytes % VECTOR_LENGTH_IN_BYTES);
